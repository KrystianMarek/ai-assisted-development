...
├── considerations/  # Open trade-offs, risks, questions (not yet ADR-worthy)
...
## Project as an LLM Wiki

This repository's `doc/` tree is maintained as a **development-focused LLM Wiki** — a persistent, compounding, interlinked knowledge base that agents keep current, rather than a set of write-once files. The pattern follows Andrej Karpathy's [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f): knowledge is compiled once and *kept current*, not re-derived on every question. The synthesis, cross-references, and flagged contradictions already exist; every source added and question asked makes the wiki richer.

### Three layers

| Layer | What | Where |
|---|---|---|
| **Raw sources** | Immutable source of truth. Agents read from these, never rewrite them. | `doc/sources/`, `doc/guidance/`, `doc/inbox/` |
| **The wiki** | Agent-maintained, interlinked markdown: synthesis, goals, status, decisions, concept pages. | `doc/overview.md`, `doc/index.md`, `doc/log.md`, `doc/goals.md`, `doc/status.md`, and the topical subdirs |
| **The schema** | This file — the conventions and workflows that make agents disciplined wiki maintainers. | `AGENTS.md` (`CLAUDE.md` symlink) |
...
Every wiki page links the related pages that give it context (use each README's "Related" section as the model). **No orphans:** every page must have at least one inbound link and should be reachable from `doc/index.md`. When you create or move a page, add the inbound links in the same change.

### Relationship to `bd`

`bd` and the wiki are complementary, not redundant:

- **`bd`** tracks *discrete work items* — bugs, features, tasks — with dependencies, claims, and status. It is the execution tracker.
- **The wiki** holds the *synthesized narrative* — why, how, and the current state — that outlives any single ticket.

Cross-link both ways: reference `bd` ticket IDs from wiki pages (e.g. `status.md`), and point tickets back at the relevant doc via `--external-ref` or a doc path in the description. Detected bugs live in `bd` (`--type bug`); their narrative post-mortems live as dated docs in `doc/development/`.
